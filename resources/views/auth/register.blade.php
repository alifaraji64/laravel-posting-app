...
@section('content')
    <div class="flex justify-center">
        <section class="bg-white p-6 w-6/12 rounded-lg flex justify-center">
            <form action="{{ route('register') }}" method="post" class="w-1/2">
                @csrf
                <div class="mb-4">
                    <label for="name" class="sr-only">Name</label>
                    <input type="text" name="name" id="name" placeholder="Your name" class="bg-gray-100 border-2 w-full p-4 rounded-lg @error('name') border-red-400 border-solid @enderror" 
                    value="{{ old('name') }}"
                    />
                    @error('name')
